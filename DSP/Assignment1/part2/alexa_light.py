...
class alexa:
    ...
    def __load_model(self):
        ...
        for i in range(len(self.model_data_frames)):
            for j in range(len(self.model_data_frames[i])):
                    temp = abs(np.fft.fft(self.model_data_frames[i][j]))[0 : self.Samples_per_frame >> 1] / self.Samples_per_frame
    ...
    """
    def __apply_window(self, data_in):
        temp = []
        # rectangular window
    ...
    """
    def __compare_with_model(self, sample, model):
        ...
        for i in range (length):
            model_frame = model[i]
            sp_sample_frame = abs(np.fft.fft(sample[i]))[0 : self.Samples_per_frame >> 1] / self.Samples_per_frame
    ...
    """
    def __detect(self, data_input):
        self.sample = data_input
        self.sample_frames = self.__apply_window(self.sample)
        score = []
        for i in range(len(self.model_data_frames_sp)):
            score.append(self.__compare_with_model(self.sample_frames, self.model_data_frames_sp[i]))
            # print(score)
    ...
    def process(self):
        ...
        raw_data = []
        shorts = []
        format = "%dh"%(self.Samples_per_frame)

        # open the audio stream
        audio_handler = pyaudio.PyAudio()
        stream = audio_handler.open(format = self.DataFormat,
        ...
        while True:
            ...
            if(max(th_data) > self.threshold):
                ...
                while(chunks_remaining >= 0):
                    raw_data.append(stream.read(self.Samples_per_frame))
                    shorts.append(struct.unpack(format, raw_data[len(raw_data) - 1]))
                    chunks_remaining -= 1
                    if(max(shorts[len(shorts) - 1]) > self.threshold):
                        chunks_remaining += 2
                ...
                if(len(wave) >= 0.2 * self.SamplingRate):
                    self.__detect(wave)
