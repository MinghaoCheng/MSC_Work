...
datapath ctlsigs memdat = (ma,md,cond,ready,a,b,ir,pc,ad,ovfl,r,x,y,p)
  where
...
-- The ALU
      (ovfl,r) = alu n (ctl_alu_a ctlsigs, ctl_alu_b ctlsigs,
                       ...
                       x y
-- The multiplier
      (ready,prod) = multiply n (ctl_mul_start ctlsigs) x y
...
-- Internal processor signals
      ...
      p  = mux1w (ctl_rf_pc ctlsigs)                -- regfile data input
             (mux1w (ctl_rf_alu ctlsigs) memdat (mux1w (ctl_r_prod ctlsigs) r (field prod 16 16)))
             pc
